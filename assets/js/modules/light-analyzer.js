class LightAnalyzer {
    ...
    _analyze() {
        ...
        try {
            ...
            const cond = brightness > bright ? 'bright' : (brightness < dark ? 'dark' : 'normal');
    ...
    _emit(event, ...args) {
        ...
        if (!list) return;
        list.forEach(fn => { try { fn(...args); } catch (e) { console.warn('LightAnalyzer CB error', event, e); } });
    }
    ...
    getLatest() { return { brightness: this.lastBrightness, condition: this.lastCondition }; }
