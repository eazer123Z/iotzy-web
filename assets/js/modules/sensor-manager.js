...
function renderSensorCard(sensorId) {
  ...
  if (sensor.type === "presence") {
    const isD = !!val;
    bodyHTML = `<div class="presence-status"><div class="presence-dot${isD ? " detected" : ""}" id="presence-dot-${sensorId}"></div><div class="presence-text" id="presence-txt-${sensorId}">${isD ? "Terdeteksi" : "Tidak Terdeteksi"}</div></div><div class="sensor-meta"><span class="sensor-meta-val" id="time-${sensorId}">—</span></div>`;
    ...
    bodyHTML = `<div style="display:flex;align-items:center;gap:12px"><div style="flex:1"><div class="sensor-value-big" id="val-${sensorId}">${valStr}<span style="font-size:13px;font-weight:500">${sensor.unit || "%"}</span></div></div><div style="width:80px;height:80px;flex-shrink:0"><svg viewBox="0 0 80 80" class="gauge-svg"><circle cx="40" cy="40" r="30" fill="none" stroke="var(--border)" stroke-width="7"/><circle cx="40" cy="40" r="30" fill="none" stroke="#3b82f6" stroke-width="7" stroke-dasharray="${(pct / 100) * 188.4} 188.4" stroke-dashoffset="47.1" stroke-linecap="round" transform="rotate(-90 40 40)" id="gauge-circle-${sensorId}"/><text x="40" y="44" text-anchor="middle" font-size="14" font-weight="800" fill="var(--ink)">${hasVal ? Math.round(val) : ""}</text></svg></div></div><div class="sensor-meta"><span class="sensor-meta-val" id="time-${sensorId}">—</span></div><canvas class="sparkline-canvas" id="spark-${sensorId}"></canvas>`;
  } else {
    let pct = 0;
    if (hasVal && cfg.min !== undefined && cfg.max !== undefined) pct = Math.min(100, Math.max(0, ((val - cfg.min) / (cfg.max - cfg.min)) * 100));
    const prec = sensor.type === "brightness" ? 2 : 1;
    bodyHTML = `<div class="sensor-status-row"><span class="sensor-value-big" id="val-${sensorId}">${hasVal ? val.toFixed(prec) + (sensor.unit || "") : ""}</span><span class="sensor-meta-val" id="time-${sensorId}">—</span></div><canvas class="sparkline-canvas" id="spark-${sensorId}"></canvas><div class="progress-track"><div class="progress-fill ${cfg.barClass || "default-bar"}" id="bar-${sensorId}" style="width:${pct}%"></div></div>`;
  }
...
function renderSensors() {
  ...
  if (document.getElementById("emptySensors")) document.getElementById("emptySensors").classList.toggle("hidden", keys.length > 0);
  keys.forEach((id) => {
    grid.appendChild(renderSensorCard(id));
...
function updateSensorValueUI(sensorId) {
  ...
  if (sensor.type === "humidity") {
    ...
    if (gc) gc.setAttribute("stroke-dasharray", `${(Math.min(100, val) / 100) * 188.4} 188.4`);
  } else {
    const prec = sensor.type === "brightness" ? 2 : 1;
    if (valEl) valEl.textContent = val.toFixed(prec) + (sensor.unit || "");
    const barEl = document.getElementById(`bar-${sensorId}`);
    if (barEl && cfg.min !== undefined && cfg.max !== undefined) barEl.style.width = Math.min(100, Math.max(0, ((val - cfg.min) / (cfg.max - cfg.min)) * 100)) + "%";
  }
...
function drawSparkline(sensorId) {
  ...
  const W = canvas.clientWidth || 200, H = canvas.clientHeight || 38;
  canvas.width = W; canvas.height = H;
  const ctx = canvas.getContext("2d");
  ctx.clearRect(0, 0, W, H);
  const mn = Math.min(...history), mx = Math.max(...history), range = mx - mn || 1;
  const pts = history.map((v, i) => ({ x: (i / (history.length - 1)) * W, y: H - ((v - mn) / range) * (H - 6) - 3 }));
  const color = (SENSOR_CONFIG[STATE.sensors[sensorId]?.type] || {}).gaugeColor || "#0ea5e9";
...
function openAddSensorModal() {
  ...
  if (document.getElementById("addSensorModal")) document.getElementById("addSensorModal").classList.add("show");
}

function closeAddSensorModal() { if (document.getElementById("addSensorModal")) document.getElementById("addSensorModal").classList.remove("show"); }
...
async function saveNewSensor() {
  ...
  const res = await apiPost("add_sensor", { name, type, unit, topic });
  if (res?.success) {
    const id = String(res.id);
    const iconMap = { temperature: "fa-temperature-half", humidity: "fa-droplet", air_quality: "fa-wind", presence: "fa-user-check", brightness: "fa-sun", motion: "fa-person-running", smoke: "fa-fire", gas: "fa-triangle-exclamation" };
    STATE.sensors[id] = { id, name, type, icon: iconMap[type] || "fa-microchip", unit, topic, sensor_key: res.sensor_key };
    STATE.sensorData[id] = null; STATE.sensorHistory[id] = [];
    if (STATE.mqtt.connected && topic) { try { STATE.mqtt.client.subscribe(topic); } catch (_) {} }
    renderSensors();
    if (document.getElementById("navSensorCount")) document.getElementById("navSensorCount").textContent = Object.keys(STATE.sensors).length;
    closeAddSensorModal(); showToast("Sensor ditambahkan!", "success");
    addLog(name, "Sensor baru ditambahkan", "System", "success");
  } else { showToast(res?.error || "Gagal menambah sensor", "error"); }
}
...
function openSensorSettings(sensorId) {
  ...
  if (document.getElementById("sensorSettingModal")) { document.getElementById("sensorSettingModal").dataset.sensorId = id; document.getElementById("sensorSettingModal").classList.add("show"); }
}

function closeSensorSettings() { if (document.getElementById("sensorSettingModal")) document.getElementById("sensorSettingModal").classList.remove("show"); }
...
async function saveSensorSettings() {
  ...
  const res = await apiPost("update_sensor", { id, name, type, unit, topic });
  if (res?.success) {
    const iconMap = { temperature: "fa-temperature-half", humidity: "fa-droplet", air_quality: "fa-wind", presence: "fa-user-check", brightness: "fa-sun", motion: "fa-person-running", smoke: "fa-fire", gas: "fa-triangle-exclamation" };
    STATE.sensors[id] = { ...STATE.sensors[id], name, type, icon: iconMap[type] || "fa-microchip", unit, topic };
    if (STATE.mqtt.connected && topic) { try { STATE.mqtt.client.subscribe(topic); } catch (_) {} }
    renderSensors(); closeSensorSettings(); showToast("Sensor diperbarui!", "success");
  } else { showToast("Gagal memperbarui sensor", "error"); }
}

async function removeSensor(sensorId) {
  const id = String(sensorId);
  if (!confirm(`Hapus sensor "${STATE.sensors[id]?.name}"?`)) return;
  const res = await apiPost("delete_sensor", { id });
  if (res?.success) {
    ...
    showToast("Sensor dihapus", "info"); addLog(name, "Sensor dihapus", "System", "warning");
  } else { showToast("Gagal menghapus sensor", "error"); }
}
