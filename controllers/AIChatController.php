<?php
/**
 * controllers/AIChatController.php — AI Chat & Telegram Test
 */
...
function handleAIChatAction(string $action, int $userId, array $body, PDO $db): void {
    if ($action === 'ai_chat_process') {
        ...
        $parsed = parse_nl_to_action($userId, $msg, [], [], 'web');
    ...
    if ($action === 'delete_chat_history') {
        dbWrite("DELETE FROM ai_chat_history WHERE user_id=?", [$userId]);
