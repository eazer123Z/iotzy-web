...
/**
 * Membangun koneksi ke broker MQTT berdasarkan pengaturan yang ada di database atau config.
 * Mendukung fitur auto-reconnect dengan exponential backoff.
 */
function applyMQTTTemplate(type) {
  ...
  const sslEl   = document.getElementById("mqttUseSSL");

  if (!type || !brokerEl) return;

  switch (type) {
    case 'local':
      brokerEl.value = window.location.hostname;
      portEl.value   = 80;
      pathEl.value   = '/mqtt';
      sslEl.checked  = false;
      break;
    case 'hivemq':
      brokerEl.value = '872ed4a7ef134dbc807b178dc908e5b0.s1.eu.hivemq.cloud';
      portEl.value   = 8884;
      pathEl.value   = '/mqtt';
      sslEl.checked  = true;
      break;
    case 'emqx':
      brokerEl.value = 'broker.emqx.io';
      portEl.value   = 8083;
      pathEl.value   = '/mqtt';
      sslEl.checked  = false;
      break;
    case 'mosquitto':
      brokerEl.value = 'test.mosquitto.org';
      portEl.value   = 8080;
      pathEl.value   = '/mqtt';
      sslEl.checked  = false;
      break;
